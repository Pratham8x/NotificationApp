...
const cleanup = setInterval(() => {
  const now = Date.now();
  for (const [id, entry] of requests) if (entry.reset <= now && !entry.active) requests.delete(id);
}, 60000);
cleanup.unref();
router.post('/chat', authenticate, (req, res, next) => {
  ...
  if (!entry || entry.reset <= now) {
    if (!entry && requests.size >= 10000) return res.status(503).json({success: false, message: 'AI is busy. Please try again later.'});
    entry = {count: 0, reset: now + 60000, active: false};
    requests.set(id, entry);
  }
  entry.count += 1;
  entry.active = true;
  Promise.resolve(chatWithGemini(req, res)).catch(next).finally(() => { entry.active = false; });
});
